...
describe('can pack', () => {
    it('packs a directory', (done) => {
        ...
        const tar = packer_1.pack({ '/apples': fixtureDir });
        let paths = [];
        ...
        extract.on('end', () => {
            paths = paths.sort();
            assert.deepStrictEqual(files, paths, 'should have tarred exactly the specified entries');
            console.log(paths);
            done();
        });
    });
    it('packs a directory honoring an ignore file', (done) => {
        const nodeTar = require('tar');
        const fixtureDir = path.join(__dirname, '..', '..', 'fixtures', 'project');
        const tar = packer_1.pack({ '/apples': fixtureDir }, { ignoreFiles: ['.ignore'] });
        let paths = [];
        const extract = tar.pipe(nodeTar.t());
        extract.on('entry', (e) => {
            paths.push(e.path);
        ...
        extract.on('end', () => {
            paths = paths.sort();
            assert.deepStrictEqual(files, paths, 'ignored files in globs in ignore files');
            console.log(paths);
            done();
        });
    });
    it('packs a directory honoring an ignore string', (done) => {
        const nodeTar = require('tar');
        const fixtureDir = path.join(__dirname, '..', '..', 'fixtures', 'project');
        const tar = packer_1.pack({ '/apples': fixtureDir }, { ignores: ['**/test'] });
        let paths = [];
        const extract = tar.pipe(nodeTar.t());
        extract.on('entry', (e) => {
            paths.push(e.path);
        ...
        extract.on('end', () => {
            paths = paths.sort();
            assert.deepStrictEqual(files, paths, 'ignored test files with **/test glob');
            console.log(paths);
            done();
        });
    });
    it('packs multiple directories', (done) => {
        const nodeTar = require('tar');
        const fixtureDir = path.join(__dirname, '..', '..', 'fixtures', 'project');
        const otherFixtureDir = path.join(__dirname, '..', '..', 'fixtures', 'outside-of-project');
        const tar = packer_1.pack({ '/apples': fixtureDir, '/oranges': otherFixtureDir });
        let paths = [];
        const extract = tar.pipe(nodeTar.t());
        extract.on('entry', (e) => {
            paths.push(e.path);
    ...
    it('packs a single file', (done) => {
        const nodeTar = require('tar');
        const fixtureDir = path.join(__dirname, '..', '..', 'fixtures', 'project');
        const tar = packer_1.pack({ '/apples/index.js': path.join(fixtureDir, 'index.js') });
        let paths = [];
        const extract = tar.pipe(nodeTar.t());
        extract.on('entry', (e) => {
            paths.push(e.path);
        ...
        extract.on('end', () => {
            paths = paths.sort();
            assert.deepStrictEqual(['apples/index.js'], paths, 'ignored test files with **/test glob');
            console.log(paths);
            done();
        });
    });
    it('packs a single file and directory', (done) => {
        ...
        extract.on('end', () => {
            paths = paths.sort();
            assert.deepStrictEqual(['apples/a-file.js', 'apples/index.js'], paths, 'packed directory and single file');
            // console.log(paths);
            done();
        });
    });
    it('errors when asked to pack a file that doesnt exist', (done) => {
        const nodeTar = require('tar');
        const tar = packer_1.pack({ '/apples': 'doesntexist' });
        tar.on('error', (err) => {
            assert.ok(err, 'should have gotten error packing source file that doesn\'t exist.');
            done();
        });
        tar.on('end', () => {
    ...
    it('applies backpressure when packing node_modules', (done) => {
        const tar = packer_1.pack({ '/apples': path.resolve(__dirname, '..', '..', 'node_modules') });
        ...
        const myTransform = new stream_1.Transform({
            transform(chunk, encoding, callback) {
                if (tar._debug_entries_written === TEST_AT_ENTRY) {
                    ...
                    waitForCondition((err) => {
                        assert.ok(!err, 'should pause within one second.');
                        callback(undefined, chunk);
                        setTimeout(() => {
                            assert.ok(tar._debug_entries_written > lastWritten, 'should process more entries in next 100 ms');
                            hitTest = true;
                        }, 100);
                    }, () => {
...
function waitForCondition(done, check, timeout) {
    const checks = 10;
