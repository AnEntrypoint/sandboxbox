...
async function instantiateModule(context, current, module2, imports, opts, wasiList, names, modules, linked, mutableFlags) {
  ...
  for (const { kind, module: module3, name } of requested) {
    ...
    if (nameIdx === -1) {
      ...
      if (module3 === EXTISM_ENV && name === "http_request" && promising && imports[module3][name] === context[ENV].http_request && !opts.executingInWorker) {
        ...
        for (const [key, entry] of Object.entries(contributions[EXTISM_ENV])) {
          imports[module3][key] = entry.bind(null, context);
        }
        imports[module3][name] = new Suspending(imports[module3][name]);
      }
      switch (kind) {
        case `function`: {
...
var AtomicsWaitAsync = Atomics.waitAsync || (() => {
  ...
  const w = new import_node_worker_threads.Worker(url, { execArgv: [] });
...
async function createWorker(opts, names, modules, sharedData, onworker = (_w) => {
  ...
  const worker = new import_node_worker_threads.Worker(WORKER_URL, opts.nodeWorkerArgs);
  onworker(worker);
