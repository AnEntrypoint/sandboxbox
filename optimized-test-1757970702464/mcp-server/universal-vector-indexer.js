...
function getAvailableEmbeddingProvider() {
  const providers = [];

  // Try transformers.js first (works on all platforms)
  try {
    require.resolve('@xenova/transformers');
    providers.push({ name: 'transformers', priority: 1 });
  } catch (e) {
    console.warn('[PLATFORM] Transformers.js not available');
  }

  // Platform-specific alternatives
  if (platform.isX64 && !platform.isWindows) {
    try {
      require.resolve('onnxruntime');
      providers.push({ name: 'onnxruntime', priority: 2 });
    } catch (e) {
      console.warn('[PLATFORM] ONNX Runtime not available');
    }
  }

  // Fallback to simple text search
  providers.push({ name: 'textsearch', priority: 99 });
...
async function initializeEmbeddingProvider() {
  const providers = getAvailableEmbeddingProvider();
    ...
    try {
      switch (provider.name) {
        case 'transformers':
          ...
          console.log(`[PLATFORM] Initialized transformers.js on ${platform.arch} (${platform.platform})`);
          return true;

        case 'onnxruntime':
          // ONNX Runtime initialization would go here
          console.log(`[PLATFORM] ONNX Runtime not yet implemented`);
          break;

        case 'textsearch':
          console.log(`[PLATFORM] Falling back to text search on ${platform.arch} (${platform.platform})`);
          embeddingExtractor = 'textsearch';
          return true;
      }
    } catch (error) {
      console.warn(`[PLATFORM] Failed to initialize ${provider.name}:`, error.message);
    }
  }
...
export async function syncIndex(folders, exts = DEFAULT_EXTS, ignores = DEFAULT_IGNORES) {
  ...
  for (const file of files) {
    try {
      ...
      if (stats.size > MAX_LARGE_FILE_SIZE) {
        console.log(`[SKIP] File too large: ${file} (${Math.round(stats.size / 1024)}KB)`);
        skippedFiles++;
        continue;
      }
      ...
      if (stats.size > MAX_FILE_SIZE) {
        console.log(`[TRUNCATE] Large file: ${file} (${Math.round(stats.size / 1024)}KB)`);
  ...
  console.log(`[DEBUG] Indexed ${newChunks.length} code chunks from ${files.length - skippedFiles}/${files.length} files (skipped ${skippedFiles})`);
